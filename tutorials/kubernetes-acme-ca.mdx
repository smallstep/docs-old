...
description: Getting cert-manager to trust an internal CA is, unfortunately, not trivial. Let's do it.
...
- You have initialized and started up a `step-ca` ACME instance using the steps in [our ACME server tutorial](/docs/tutorials/acme-challenge).
  Alternatively, you can use our hosted CA, [Smallstep Certificate Manager](/certificate-manager).
