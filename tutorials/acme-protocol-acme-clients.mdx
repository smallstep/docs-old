...
## Requirements
- For [Smallstep Certificate Manager](/docs/certificate-manager/acme), this tutorial assumes you have created a hosted or linked authority and are running a local [ACME Registration Authority](/docs/registration-authorities/acme-for-certificate-manager). 
- For `step-ca`, this tutorial assumes you have initialized and started up an instance using the steps in [Getting Started](/docs/step-ca/getting-started). Additionally, you'll need to configure your CA with an ACME provisioner. Run `step ca provisioner add acme --type ACME`, and restart your CA. 
