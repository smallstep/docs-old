...
If you run into any issues please let us know [in GitHub Discussions][gh-discussions].
...
* [ACME and `step-ca`](#acme-and-step-ca-recap)
* [Certbot](#certbot)
* [acme.sh](#acmesh)
* [Caddy](#caddy)
* [NGINX](#nginx)
...
To connect to `step-ca`, you need to point the client to the right [ACME Directory URL][acme-dir].
...
Communication between an ACME client and server always uses HTTPS.
Clients will validate the server’s HTTPS certificate using the public root certificates in your system’s default [trust store][pki-ts].

[pki-ts]: https://smallstep.com/blog/everything-pki/#trust-stores

### Certbot

[`certbot`](https://certbot.eff.org/) is the granddaddy of ACME clients. Built
...
In most cases you don't need to do any configuration and should follow the caddy documentation on [automatic HTTPS][auto-https].

[auto-https]: https://caddyserver.com/docs/automatic-https#local-https

If you want to set things up manually, to get a certificate from `step-ca` configure the [tls.issuance][tls-issuance] module in your Caddyfile:

* Point Caddy at your ACME directory URL using the `ca` field
* Tell Caddy to trust your root certificate using the `trusted_roots_pem_files` array

[tls-issuance]: https://caddyserver.com/docs/modules/tls.issuance.acme#module-list-container

An example module in your `Caddyfile` might resemble:
...
foo.internal {
  ...
  tls.issuance {
    ...
    "trusted_roots_pem_files": ["<step path>/certs/root_ca.crt"]
  }
}
```

Run `caddy` to start serving HTTPS!
