...
## Requirements
- **Open source -** For `step-ca`, this tutorial assumes you have initialized and started up an instance using the steps in [Getting Started](/docs/step-ca/getting-started). Additionally, you'll need to configure your CA with an ACME provisioner. Run `step ca provisioner add acme --type ACME`, and restart your CA. 
- **[Smallstep Certificate Manager](/docs/certificate-manager/acme) -** this tutorial assumes you have [created a hosted or linked authority](/docs/certificate-manager/getting-started) and are running a local [ACME Registration Authority](/docs/registration-authorities/acme-for-certificate-manager). 


## Overview

Most ACME clients connect to Let’s Encrypt’s CA by default.
To connect to smallstep, you need to point the client to the right [ACME Directory URL][acme-dir].
