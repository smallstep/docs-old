...
* `HTTP_PROXY`, `HTTPS_PROXY`, and `NO_PROXY` Configure proxies for outbound HTTPS traffic. See [net/http.ProxyFromEnvironment](https://golang.org/pkg/net/http/#ProxyFromEnvironment) documentation for details. Note that the system trust store will be used for proxy TLS handshakes.
