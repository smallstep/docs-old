...
Finally, `step`stores Alice's certificate in her laptop's SSH agent. It's never stored on disk.

From here, the `ssh handshake` continues seamlessly. <br>

![image](/uploads/ssh-hiw-alice1.png 'SSH Certificate Issued')
...
![](/uploads/acl-tag-example.png 'Host tags example')
...
![](/uploads/ssh-hiw-access-control.png 'Grant Access Example')
...
![image2](/uploads/ssh-hiw-alice2.png 'SSH Handshake') <br>
...
3. Go through an in-browser single sign-on OAuth OIDC flow to authenticate
4. SSH as normal. When your certificate expires, you’ll authenticate through the browser again.
