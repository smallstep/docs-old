...
```shell-session
# curl -sSf http://files.smallstep.com/ssh-host.sh
# sh ssh-host.sh --is-bastion
```
...
```shell-session
# curl -sSf http://files.smallstep.com/ssh-host.sh
# sh ssh-host.sh --bastion=<bastion hostname>
```
...
```shell-session
# curl -sSf http://files.smallstep.com/ssh-host.sh | sh
```
...
* Any number of `--principal="[additional hostname]"`,
to add additional hostnames to the host certificate
...
you can follow our [step-by-step installation instructions](hosts-step-by-step) instead.
...
```shell-session
# curl -sSf http://files.smallstep.com/ssh-host-uninstall.sh | sh
```
