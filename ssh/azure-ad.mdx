...
3. Add the Smallstep SSH Azure enterprise application
4. Configure SCIM credentials in Azure and enable user provisioning
...
Run `step ssh login` locally.
Your browser will open with an Azure AD single sign-on flow.
You'll be prompted to add the Smallstep SSH enterprise application to your tenant.
Approve the application for your tenant and finish the sign-on flow.
...
### Step 4. Configure SCIM credentials in Azure and enable user provisioning

1.  In the [Enterprise Applications](https://portal.azure.com/#blade/Microsoft_AAD_IAM/StartboardApplicationsMenuBlade/AllApps/menuId/) blade, you should now see Smallstep SSH. Open it.
