...
2. Move your CA configuration into a system-wide location. Run:
  ...
  but have never run it, you may also need to create an empty `/etc/step-ca/db` directory:
...
3. Create a `systemd` unit file.
   ...
   * `ConditionFileNotEmpty` will prevent the service process from starting without configuration files.
   ...
   * `PrivateTmp` adds a private `/tmp` to the process sandbox, to secure access to temporary files.
   * `PrivateUsers` restricts the service from accessing arbitrary system user information.
