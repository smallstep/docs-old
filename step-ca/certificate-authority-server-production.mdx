...
Place the following in `/etc/systemd/system/cert-renewer@.service`:
...
Then run `systemctl daemon-reload` to reload the service unit files.
...
Place the following in `/etc/systemd/system/cert-renewer@.timer`:
