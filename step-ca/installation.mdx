...
- The `step-ca` binary tarball can be found [here](https://archlinux.org/packages/community/x86_64/step-ca/).
...
1. Install `step`
    ...
    ```shell
    wget -O step.tar.gz https://github.com/smallstep/cli/releases/download/v0.15.14/step_linux_0.15.14_amd64.tar.gz
    tar -xf step.tar.gz
    sudo cp step_0.15.14/bin/step /usr/bin
    ```
...
2. Install `step-ca`
    ...
    ```shell
    wget -O step-ca.tar.gz https://github.com/smallstep/certificates/releases/download/v0.15.11/step-ca_linux_0.15.11_amd64.tar.gz
    tar -xf step-ca.tar.gz
    sudo cp step-ca_0.15.11/bin/step-ca /usr/bin
    ```
