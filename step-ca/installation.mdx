...
There are community-maintained packages for NixOS 20.09:
- [`step-ca`](https://search.nixos.org/packages?channel=20.09&from=0&size=50&sort=relevance&query=step-ca)
- [`step-cli`](https://search.nixos.org/packages?channel=20.09&show=step-cli&from=0&size=50&sort=relevance&query=step-cli)

The NixOS `unstable` channel hosts more recent versions of [`step-cli`](https://search.nixos.org/packages?channel=unstable&show=step-cli&from=0&size=50&sort=relevance&query=step-cli) and [`step-ca`](https://search.nixos.org/packages?channel=unstable&show=step-ca&from=0&size=50&sort=relevance&query=step-ca).
...
[systemd-setup]: certificate-authority-server-production#running-step-ca-as-a-daemon
