...
Many SCEP client implmentations require an RSA CA chain. Because `step ca init` creates ECDSA chains by default, you may need to [use an RSA CA chain](/docs/tutorials/rsa-chain) in your CA to ensure broader compability.
...
Next, you will need the CA to listen using HTTP, which it does not do by default. Enable this by filling in the `"insecureAddress"` property to your top-level CA configuration:
...
Your SCEP provisioner is now available at the endpoint `http://your.ca.hostname:8080/scep/scepca`.
