...
- [Cryptographic Protection](#cryptographic-protection)
  ...
  - [AWS KMS](#aws-kms)
  - [Azure Key Vault KMS](#azure-key-vault-kms)
...
To configure Azure Key Vault in your certificate authority, add the `kms` property to `ca.json` and replace the `key` property with the key name of your intermediate key:

```json nocopy
{
    "root": "/home/step/certs/root_ca.crt",
    "crt": "/home/step/certs/intermediate_ca.crt",
    "key": "azurekms:key-id=f879f239-feb6-4596-9ed2-b1606277c7fe",
    "kms": {
        "type": "azurekms",
        "uri": "azurekms:name=key-name;vault=vault-name?hsm=true"
    }
}
```

Authentication to Azure is handled via environment variables; we recommend [using file-based authentication](https://docs.microsoft.com/en-us/azure/developer/go/azure-sdk-authorization#use-file-based-authentication) via the `AZURE_AUTH_LOCATION` environment variable. See [Authentication Methods for Azure SDK for Go](https://docs.microsoft.com/en-us/azure/developer/go/azure-sdk-authorization) for an overview of authentication types and methods.
