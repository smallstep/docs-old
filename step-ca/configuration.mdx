...
You'll learn how to configure your CA to:
...
When you run `step-ca`, you must provide a path to a configuration file (`ca.json`). 
It is the only required argument. Typically you'd use `step path` to discover the path to `ca.json`:
...
- **kms**: enables and configures [cryptographic protection](#cryptographic-protection) using cloud key management services or hardware security modules.
  ...
  - **type**: required. Valid values are: `awskms`, `cloudkms`, `pkcs11`, or `yubikey`
...
- **address**: ex. 127.0.0.1:9000 - address and port on which the CA will bind and respond to requests.
...
- **db**: data persistence layer. See [database documentation](#databases) for more info.
  ...
  - **type**: Valid values are: badger, badgerV1, badgerV2, bbolt, mysql.
...
- **tls**: settings for negotiating communication with the CA; includes [acceptable ciphersuites](https://ciphersuite.info/), min/max TLS version, renegotiation.
...
Provisioners are methods of proving a client identity to the CA prior to getting a new certificate.
Visit the [`step-ca` provisioners](/docs/step-ca/provisioners) page to learn about the different provisioners, their target use cases, and how to add, remove, and configure them.
