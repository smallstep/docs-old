...
The current implementation supports X5C and JWK provisioners for communication between RAs and the CA.
The RA can use any provisioners with its clients.


The configuration of the RA can look like
```json
{
	"address": ":9100",
...
To configure it with a JWK provisioner we'll change the `"certificateIssuer"` property to something like:
```json
{
	"type": "jwk", 
...
If either the x5c key or the jwk key is password-protected, the password can be given in the configuration like
```json
{
	"type": "jwk", 
	"provisioner": "mariano@smallstep.com",
