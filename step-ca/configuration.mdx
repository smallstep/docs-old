...
Unfortunately, an IID usually doesn't contain enough information to generate a complete certificate a the host.
...
`step-ca` has some mechanisms that strengthen these security characteristics:
- A cloud provisioner will only grant a certificate _once per host_.
  After this first grant, the host will need to renew the certificate using mTLS (`step ca renew`).
  If the certificate ever expires, you'll need to use a different provisioner to issue a new one.
- If you configure a max instance age (using `instanceAge`, below),
  and your infrastructure is setup such that some trusted process (e.g., cloud-init config management),
  that runs when every instance is launched,
  to "consume" the instance identity document and obtain a certificate,
  then you may have more assurance that the bound names in certificates are legitimate.
- If you disable custom SANs (using `disableCustomSANs`, below) then the CA will only issue certificates with authentic name(s) extracted from the signed instance identity document.
  Unfortunately, the names on the IID may not be the names that you use to refer to your servers and services.

Keep in mind that, in this model, instances that don't obtain a certificate are a risk:
Anyone with access to the instance will be able to obtain a certificate binding any name,
