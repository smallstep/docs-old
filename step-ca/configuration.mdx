...
- [Cloud Key Management Service](#cloud-key-management-service)
  - [Google's Cloud KMS](#googles-cloud-kms)
  - [AWS KMS](#aws-kms)
  - [YubiKey](#yubikey)
...
Occasionally it’s useful to issue certificates to people (ex., for testing and
debugging). To facilitate this, `step-ca` can be configured to use
OAuth/OpenID Connect (OIDC) identity tokens for authentication. In other words,
you can use single sign-on with G Suite, Okta, Active Directory, or any
other OAuth OIDC provider to get a certificate.
...
For cloud VMs running on AWS, GCP, or Azure `step-ca` can be configured to use
instance identity documents (IIDs) to authorize certificate signing requests.
IIDs make integrating `step-ca` even easier.

Instead of building and integrating a JWK provisioner that issues tokens to
workloads, `step` can request tokens directly from your cloud provider’s
metadata API with no additional infrastructure requirements.
...
Instance Identity is implemented as a signed document (see an
example GCP instance identity token [here][2]) encoding host information like
_name_ and _owner_. The information contained in an IID may not be enough
to generate a certificate. Due to this limitation, the cloud provisioners default
to using a trust model called _Trust On First Use (TOFU)_.

The _Trust On First Use_ model allows the use of more permissive certificate
signing requests (CSRs) that can have custom SANs that cannot be validated. It
comes with the limitation that you can only grant a certificate once per host.
After this first grant, the same machine will need to renew the certificate
using mTLS and the CA will block any other attempt to grant a certificate to
that instance.
...
- **instanceAge**<Reference id="star8" marker="*" />: the maximum age of an instance to grant a
...
- **instanceAge**<Reference id="star9" marker="*" />: the maximum age of an instance to grant a
  certificate. The instance age is a string using the duration format.
...
People use private CAs for all sorts of things, in many different contexts: web
apps, mobile apps, code signing, cloud VM instances, SSH, IoT devices, etc. So
`step-ca` must be flexible enough to handle a wide variety of flows.

That's why, beginning with `step` and `step-ca` version `v0.15.0` we are
introducing the concept of certificate templates.

With X.509 templates, you can do things like:

- Add custom SANs to certificates.
- Add custom X.509 extensions to certificates.
- Make longer certificate chains, with multiple intermediate CAs.
- Add conditionals around any of a certificate's parameters, and fail if they are not met.

### Basic Templates

X.509 templates are JSON documents that describe the most important fields in
the final certificate or certificate request. Templates use Go's
[text/template](https://golang.org/pkg/text/template/) at the core and are
extended with [Sprig functions](http://masterminds.github.io/sprig/).

X.509 templates can be used with `step` and `step-ca`. With `step`, 
templates are generally used by passing `--template <path>` to the `step
certificate create` or `step certificate sign` subcommands. With `step-ca`,
templates are configured per provisioner.

Within provisioner configuration, a certificate template can be set under the new
property `"options"`. The following snippet shows a provisioner with a custom
template:
...
- **options**: object that allows configuration of provisioner options e.g. templates.
  ...
  - **x509**: object for configuring X.509 certificate options.
    ...
    - **templateData**: defines variables that can be used in the
      template. In the example above you will be able to use the defined
      organizational unit as the variable `{{ .OrganizationalUnit }}`, for example in
      a template like:
...
You will learn the most common templates in the following subsections and
advance to more complex templates later.
...
This template is most commonly used in the `step certificate create` command
when the flag `--profile intermediate-ca` is set. But it can also be set in the
...
In the previous section, we learned how to create and use the basic templates, but
we've just scratched the surface of what's possible. With templates, you can 
customize and parameterize every aspect of an X.509 certificate.

For example, you can:

- extend path lengths
- add custom extensions
- define name constraints for your intermediates
- add custom information provided by the user
- define what your certificate can be used for (e.g. **_codeSigning_**)
- etc.

Below we'll walk through a few more advanced templating examples.
...
In `step-ca`, templates can also be parameterized with variables that will be
...
For example, below is a template that accepts the user variable `dnsName`
...
{{- if .Insecure.User.dnsName }}
	"dnsNames": "{{ .Insecure.User.dnsName}}",
{{- else }}
	"sans": {{ toJson .SANs }},
{{- end }}
{{- if typeIs "*rsa.PublicKey" .Insecure.CR.PublicKey }}
...
More information and more examples are available in the blog post ["Announcing
X.509 Certificate
Flexibility"](https://smallstep.com/blog/x509-certificate-flexibility/).
...
Support for multiple KMS are planned, but only Google's Cloud KMS and Amazon's
AWS KMS are currently supported. An [experimental version for YubiKeys](#yubikey) is also
available.
...
You can use a hardware [YubiKey](https://www.yubico.com/)—and the YubiKey PIV application—to store your CA keys and sign TLS certificates.
...
`step-ca` uses [piv-go](https://github.com/go-piv/piv-go) and requires PCSC support. This is available by default on macOS and Windows operating systems, but on Linux piv-go requires PCSC lite.
...
<CodeBlock
    language="shell"
    copyText="sudo yum install 'dnf-command(config-manager)' && sudo yum config-manager --set-enabled PowerTools && sudo yum install pcsc-lite-devel"
  >{`sudo yum install 'dnf-command(config-manager)'
sudo yum config-manager --set-enabled PowerTools
sudo yum install pcsc-lite-devel`}</CodeBlock>
