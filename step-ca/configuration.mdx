...
Certificate templates open up these possibilities.
With certificate templates, you can do things like:

- Add custom SANs to X.509 certificates
- Use custom X.509 and SSH certificate extensions
...
* **.Token** If a signed token was used to obtain the certificate 
...
* **.Insecure.CR.PublicKey** The public key provided in the certificate request.
...
You can also import parameter values from your provisioner's [configuration block](#configuring-the-ca-to-use-templates).

### SSH Templates

SSH templates can be used with `step-ca`.
Within the current OpenSSH feature set, SSH user certificates offer the most flexibility.
Here's the default [SSH user certificate template](https://github.com/smallstep/crypto/blob/162770cad29063385cb768b0191814e4c6a94e45/sshutil/templates.go#L144):
...
* **.Token** If a token was used to obtain the certificate 
  —for example, with the OIDC or JWK provisioners—
...
* **.Principals** For users, this value is derived from the OIDC token
  ...
  (when using the IID provisioners).
* **.Insecure.CR** This field holds the Certificate Request from the client.
