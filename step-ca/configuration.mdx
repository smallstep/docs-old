...
You will be walked through the process of creating root and intermediate CA signing keys in Key Vault, and configuring the CA to use them. If you create an SSH CA, SSH host and user CA keys will be created in Key Vault as well.
...
* In the `key` URI, the `name` and `vault` should point to the vault name and key name of your intermediate key in Azure Key Vault.
