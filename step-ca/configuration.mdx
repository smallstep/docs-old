...
before passing them along to an upstream Certificate Authority that will issue the certificates.
...
Let's set up a simple RA <-> CA configuration, with one RA and one CA.
...
The RA, in turn, can use any `step-ca` provisioners with its clients.
Configure RA provisioners just as you would configure provisioners for a CA.

#### Setting up the CA

You can set up the CA using the [Getting Started](./getting-started) guide.
You will need the name and password of the first provisioner that you create during the `step ca init` process.

#### Setting up the RA

The current implementation supports the JWK and X5C provisioners for communication between RAs and the CA.
The JWK provisioner is recommended, as it covers the most common use case.
...
This connects this RA to the CA using the JWK provisioner named `ra@smallstep.com` on the CA.
When `step-ca` starts up with this configuration, it will download the encrypted JWK key from the CA automatically.
The RA will still need the JWK provisioner password, to decrypt the key.
The JWK password can be passed in via a file when starting the RA:

```bash
$ step-ca --issuer-password-file=$(step path)/secrets/issuer_jwk_password.txt $(step path)/config/ca.json
```

That's it. Your RA is ready to process ACME requests.

#### Adding the issuer's password to `ca.json`

If `--issuer-password-file` isn't to your liking, you can provide an issuer `"password"` in the `"certificateIssuer"` configuration block:
...
This works for upstream JWK or X5C provisioners.

#### Using the X5C provisioner

To configure an RA <-> CA pair to use an X5C provisioner,
first add an X5C provisioner to the CA.

See the [X5C Provisioner](#x5c-x509-certificate) configuration section for more details.

Then, change the `"certificateIssuer"` object on the RA as follows:

```json
{
  "type": "x5c", 
  "provisioner": "X5C",
  "crt": "/etc/step-ca/x5c.crt",
  "key": "/etc/step-ca/x5c.key"
}
```		

The X5C provisioner named `X5C` in the CA will need to be configured to accept the `x5c.crt` and private key.
...
5. Initialize your PKI.
  ...
  Note that `"credentialsFile"` isn't the only way you can provide Google Cloud `credentials.json` filename to the RA.
...
6. Start `step-ca`
  ...
  and because the intermediate is in Google Cloud, `"crt"` and `"key"` are not needed.
