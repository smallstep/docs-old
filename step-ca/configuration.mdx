...
- [Cryptographic Protection](#cryptographic-protection)
  ...
  - [AWS KMS](#aws-kms)
  - [Azure Key Vault](#azure-key-vault-kms)
...
When using Azure Key Vault with `step-ca`, authentication to Azure is handled via environment variables; we recommend using either [file-based authentication](https://docs.microsoft.com/en-us/azure/developer/go/azure-sdk-authorization#use-file-based-authentication) via the `AZURE_AUTH_LOCATION` environment variable, or setting the `AZURE_TENANT_ID`, `AZURE_CLIENT_ID`, and `AZURE_CLIENT_SECRET` variables when starting `step-ca`. See [Authentication Methods for Azure SDK for Go](https://docs.microsoft.com/en-us/azure/developer/go/azure-sdk-authorization) for an overview of authentication methods and environment variables.
...
You will be walked through the setup and configuration process.
...
* In `key`, the `name` and `vault` should point to the vault name and key name of your intermediate key in Azure Key Vault.
* In `key`, the `version` is optional. If omitted, the latest version will be used.
* In `key`, the optional `hsm` property can be set to `true` if HSM protection is desired. The default is `false`.
* In `kms`, the `uri` can be used to provide client credentials (eg. `azurekms:client-id=fooo;client-secret=bar;tenant-id=9de53416-4431-4181-7a8b-23af3EXAMPLE`), but this is not recommended.
...
You will need the name and password of the first provisioner you created during the `step ca init` process.
