...
you can create a new key for your RA by decrypting the CA's provisioner key and re-encrypting it with a new password for the RA.
...
As before, use `--issuer-password-file` to provide the encrypted key password when starting up the CA.
...
In this scenario, the RA will need to be configured with a certificate and key signed by the root configured in the CA's X5C provisioner.
