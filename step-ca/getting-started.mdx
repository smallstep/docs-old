...
It's not always practical or desirable to run the CA as a daemon.
And, you don't always need to!
`step` offline mode leverages the CA's configuration, which must be available locally, to issue certificates.
Just use the `--offline` flag:
...
`step` can also generate bootstrap tokens in offline mode and exchange them with a running CA for a certificate.
...
You can use the `step ca` command group to interact with the CA.
