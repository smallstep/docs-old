...
description: ACME Registration Authority for GCP CAS
---
...
#### 2. Create a CA Pool
  ...
  Run:
   ...
   ```shell
   gcloud privateca pools create kevin-ca-pool --location us-west1
  ```
...
To create, run:
  ...
  ```shell
   gcloud privateca roots create kevin-root-ca \
       --pool kevin-ca-pool \
       --location us-west1 \
       --subject "CN=Kevin Root CA, O=Kevin LLC" \
...
To enable CA, run:
  ...
  ```
    gcloud privateca roots enable kevin-root-ca \
      ---pool kevin-ca-pool \
      --location us-west1 
  ```
...
#### 4. Create and Enable an Intermediate CA.
  ...
  To create, run:
   ...
   ```shell
   gcloud privateca subordinates create kevin-intermediate-ca \
       --pool kevin-ca-pool \
       --location us-west1 \
       --issuer-ca kevin-root-ca \
       --issuer-pool kevin-ca-pool \
       --issuer-location us-west1 \
       --subject "CN=Kevin Intermediate CA, O=Kevin LLC" \
       --key-algorithm ec-p256-sha256
   ```
  ...
  To enable, run:
   ...
   ```shell
   gcloud privateca subordinates enable kevin-intermediate-ca \
       --pool kevin-ca-pool \
       --location us-west1
   ```


#### 5. Configure a service account with necessary role bindings to request and manage X.509 Certificates using CAS. 
  ...
  ![](/static/images/cas-required-fields.png)
    ...
    * **CAS Certificate Authority ID**: name of the CAS intermediate Certificate
      Authority created above.
        ...
        ```shell
        gcloud privateca subordinates describe kevin-intermediate-1 --pool kevin-pool-1 --location us-west1 --format='get(name)'
        ```
...
3. Optional fields in the Launcher UI.
  ...
  ![](/static/images/cas-optional-fields.png)
...
4. Deploy!
  ...
  ![](/static/images/cas-deploy.png)

#### 7. Let's test out our deployment!
...
3. Download the root certificate.
  ...
  ```shell
  gcloud privateca roots describe kevin-ca-1 --pool kevin-pool-1 --location us-west1 --format='get(pemCaCertificates)' > /tmp/root_ca.crt
  ```
...
4. Create a new certificate using the ACME RA for CAS.
  ...
  # Get a certificate!
  $ sudo REQUESTS_CA_BUNDLE=/tmp/root_ca.crt certbot certonly -n --standalone -d $HOST --server https://acme-registration-authority-for-gcp-c-1-vm.c.smallstep-public.internal/acme/acme/directory --agree-tos --email tester@smallstep.com
  ```
