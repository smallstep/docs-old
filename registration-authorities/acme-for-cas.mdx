...
#### 2. Create a Root CA.
  ...
  <Alert severity="info">
    <div> 
      See <Code>gcloud beta privateca roots create --help</Code> for more options.
...
#### 3. Create an Intermediate CA.
  ...
  Run:
   ...
   ```shell
   gcloud beta privateca subordinates create example-intermediate-ca \
       --location us-west1 \
...
#### 4. Configure a service account with necessary role bindings to request and manage X.509 Certificates using CAS. 
...
#### 5. Launch a "Smallstep ACME Registration Authority for CAS" deployment
  ...
  ![](/static/images/cas-required-fields.png)
    ...
    * **CAS Certificate Authority ID**: name of the CAS intermediate Certificate
      Authority created above.
        ...
        ```shell
        gcloud beta privateca subordinates describe example-intermediate-ca --location us-west1 --format='get(name)'
        ```
...
#### 6. Let's test out our deployment!
...
3. Download the root certificate.
  ...
  ```shell
  gcloud beta privateca roots describe example-root-ca --location us-west1 --format='get(pemCaCertificates)' > /tmp/root_ca.crt
  ```
...
4. Create a new certificate using the ACME RA for CAS.
  ...
  # Get a certificate!
  $ sudo REQUESTS_CA_BUNDLE=/tmp/root_ca.crt certbot certonly -n --standalone -d $HOST --server https://acme-registration-authority-for-gcp-cas-1-vm.c.$PROJECT_ID.internal/acme/acme/directory --agree-tos --email tester@smallstep.com
  ```
