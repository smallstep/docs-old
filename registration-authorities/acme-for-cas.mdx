...
#### 2. Create a CA Pool and a Root CA.
...
#### 3. Create an Intermediate CA.
  ...
  Run:
   ...
   gcloud privateca subordinates create example-intermediate-ca \
       ...
       --key-algorithm ec-p256-sha256 \
       --use-preset-profile "subordinate-server-tls-pathlen-0"
   ```
  ...
  ![](/static/images/cas-required-fields.png)
    ...
    * **CAS Certificate Authority ID**: name of the CAS intermediate Certificate
      Authority created above.
        ...
        ```shell
        gcloud privateca subordinates describe example-intermediate-ca --location us-west1 --format='get(name)'
        ```
...
3. Download the root certificate.
  ...
  ```shell
  gcloud privateca roots describe example-root-ca --location us-west1 --format='get(pemCaCertificates)' > /tmp/root_ca.crt
  ```
