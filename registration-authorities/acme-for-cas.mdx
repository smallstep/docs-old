  ...
  ![](/static/images/cas-required-fields.png)
    ...
    * **CAS Certificate Authority ID**: name of the CAS intermediate Certificate
      Authority created above.
        ...
        ```shell
        gcloud privateca subordinates describe example-intermediate-1 --pool example-pool-1 --location us-west1 --format='get(name)'
        ```
...
3. Download the root certificate.
  ...
  ```shell
  gcloud privateca roots describe example-ca-1 --pool example-pool-1 --location us-west1 --format='get(pemCaCertificates)' > /tmp/root_ca.crt
  ```
