...
import DynamicDocForm from 'docs/DynamicDocForm';

export const router = () => useRouter();

export const query_string = (queryParam, defaultValue) => router().query[queryParam] && router().query[queryParam].length ? router().query[queryParam] : defaultValue;
export const ca_url = () => router().query.caUrl && router().query.caUrl.length ? router().query.caUrl : "https://ra.example.com"
export const fingerprint = () => router().query.fingerprint && router().query.fingerprint.length  ? router().query.fingerprint : "b4fc6b547ca4610b69cfcc53c6933e7a37170476dfe13EXAMPLE92c403f5"
export const provisioner = () => router().query.provisioner && router().query.provisioner.length ? router().query.provisioner : "acme-ra-jwk"
  export const address = () => router().query.address && router().query.address.length ? router().query.address : ":443"
export const dns = () => {
  return router().query.dns && router().query.dns.length ? router().query.dns : "ra.example.com"
}
...
<DynamicDocForm
  ...
  baseUrl="/docs/registration-authorities/acme-for-certificate-manager"
  title="Use this form to pre-fill some of the example commands"
/>
...
1. Create an Authority in Certificate Manager with a JWK provisioner
    ...
    <CodeBlock language="shell-session" copyText={ca_config()}>
      <> $ step beta ca provisioner add {provisioner()} --type JWK --create</>
    </CodeBlock>
    ```bash
    $ step beta ca provisioner add acme-ra-jwk --type JWK --create
    ```
...
2. Setup a local instance of `step-ca` that can act as a registration authority (RA).
    ...
    You'll need to configure the RA to work with your hosted Authority in Certificate Manager, using the JWK provisioner you just created for the RA to Authority connection.
      ...
      Create a `/etc/step-ca/config/ca.json` configuration file with the following contents:
...
3. Distribute your root CA certificate and start issuing certs. 🥳
    ...
    <CodeBlock language="shell-session" copyText="step certificate inspect server.crt">
      <>
      $ step ca root root_ca.crt --ca-url {ca_url()} --fingerprint {fingerprint()}
      </>
    </CodeBlock>
