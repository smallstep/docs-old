...
- Administrator privleges on your Smallstep Authority
- IDP priveldges capable of creating a new OIDC applicatoin integration and assigning users to the integration. 
...
This command requires Administrator privleges on the Authority. The terminal will prompt you to enter your email and perform a single-sign on flow via the smallstep dashboard. 
...
Upon completion your OIDC provisioner will be created and will show up in your smallstep dashboard authority detail page. 

<Alert severity="info">
  <div>
    OKTA requires that you specify a port for the listerner. You will need to add the following flag to the provisioner add command `--listen-address 127.0.0.1:10000`
  </div>
</Alert>
...
Once updated, the provisioner admin can request a certificate with any SAN
...
Create a [Smallstep account](https://smallstep.com/signup?product=cm) and start issuing self-serve single sign-on certifcates to all your developers. Prefer a human? [Schedule time](https://smallstep.com/acme-form/) to talk with Smallstep. 
